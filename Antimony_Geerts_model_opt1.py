...
def run_optimization_and_simulation():
    ...
    def create_objective(csv_data, r):
        def objective(params):
            IDE_activity_ISF_val, k_APP_production_val = params

            # Reload model and set parameters
            r.reset()
            r['k_O24_O12_AB42_ISF'] = IDE_activity_ISF_val
            ...
            print(mse, IDE_activity_ISF_val, k_APP_production_val)
            return mse
        return objective
    ...
    initial_guess = [r['k_O24_O12_AB42_ISF'], r['k_APP_production']]
    print(initial_guess)
    # Bounds (set as appropriate)
    bounds = [(1, 1000), (1, 1000)]  # Adjust as needed

    # Run optimization
    opt_result = minimize(objective, initial_guess, bounds=bounds, method='Nelder-Mead')
    print("Optimized k_O24_O12_AB42_ISF:", opt_result.x[0])
    print("Optimized k_APP_production:", opt_result.x[1])

    # Update model with optimized parameters
    r.reset()
    r['k_O24_O12_AB42_ISF'] = opt_result.x[0]
    r['k_APP_production'] = opt_result.x[1]
    print(r['k_O24_O12_AB42_ISF'], r['k_APP_production'], opt_result.x[0], opt_result.x[1])
