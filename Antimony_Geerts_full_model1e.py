...
def setup_model_and_simulate(r, param_names, param_values, microglia_params, ab42_params, ab40_params):
    ...
    try:
        ...
        result = r.simulate(20*365*24, 100*365*24, 7000, ['time', 
            ...
            '[Antibody_central]', '[Antibody_SAS]', '[AB42_O25__Antibody_ISF]'])
...
def plot_antibody_ISF(ax, result1, result2, time_years1, time_years2, ab_type):
    ...
    column_name = f'[AB42_O25__Antibody_ISF]'
    if column_name not in result1.colnames:
        print(f"Warning: Column {column_name} not found in simulation results")
        print(f"Available columns: {result1.colnames}")
        # Plot a placeholder or skip this plot
        ...
        return
    ax.plot(time_years1, result1[column_name], label=f'{ab_type}_ISF ApoE', linewidth=2, color='red')
    ax.plot(time_years2, result2[column_name], label=f'{ab_type}_ISF non-ApoE', linewidth=2, color='blue')
    ax.set_xlabel('Time (years)')
    ax.set_ylabel('Concentration (nM)')
    ax.set_title('Antibody ISF')
    ax.legend(loc='upper left')
    ax.set_xlim(70, 73)
    ax.grid(True)
...
def create_standard_plots(result1, result2, csv_data_3C_ApoE, csv_data_3C_nonApoE, 
    ...
    if ab_type == 'AB42':
        ...
        plot_antibody_ISF(axes[1, 0], result1, result2, time_years1, time_years2, ab_type)
    ...
    if ab_type == 'AB42':
        ...
        plot_antibody_central(axes[1, 1], result1, result2, time_years1, time_years2, ab_type)
    ...
    if ab_type == 'AB40':
        ...
        plot_antibody_SAS(axes[2, 0], result1, result2, time_years1, time_years2, ab_type)
