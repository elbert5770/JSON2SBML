...
#   Baseline_AB42_O_P: 0.0422077048633233
    ...
    r['Microglia_EC50_AB40'] = Microglia_EC50_AB40_APOE4
    r['Microglia_Vmax_AB40'] = Microglia_Vmax_AB40_APOE4
    rates = calculate_k_rates(r['k_O1_O2_AB42_ISF'], r['k_O2_O3_AB42_ISF'], r['k_O2_O1_AB42_ISF'], r['k_O3_O2_AB42_ISF'],forAsymp4x=2.0, forHill4x=3.0, backAsymp4x=2.0, backHill4x=3.0)
    print(rates)
    oligomer_sizes = list(range(4, 25))
    for i, size in enumerate(oligomer_sizes):
    ...
    rates = calculate_k_rates(r['k_O1_O2_AB40_ISF'], r['k_O2_O3_AB40_ISF'], r['k_O2_O1_AB40_ISF'], r['k_O3_O2_AB40_ISF'],forAsymp4x=0.3, forHill4x=2.0, backAsymp4x=0.3, backHill4x=2.0)
...
# Update model with optimized parameters
    ...
    r['Microglia_EC50_AB40'] = Microglia_EC50_AB40_nonAPOE4
    r['Microglia_Vmax_AB40'] = Microglia_Vmax_AB40_nonAPOE4
    rates = calculate_k_rates(r['k_O1_O2_AB42_ISF'], r['k_O2_O3_AB42_ISF'], r['k_O2_O1_AB42_ISF'], r['k_O3_O2_AB42_ISF'],forAsymp4x=2.0, forHill4x=3.0, backAsymp4x=2.0, backHill4x=3.0)
    print(rates)
    oligomer_sizes = list(range(4, 25))
    for i, size in enumerate(oligomer_sizes):
...
def create_plots(r, result1, result2, csv_data_3C_ApoE, csv_data_3C_nonApoE, csv_data_3A_ApoE, csv_data_3A_nonApoE, suvr, csv_data_3B_ApoE, csv_data_3B_nonApoE):
    ...
    ax6.set_ylabel('Concentration (nM)')
    ax6.set_title('Plaque')
    ax6.legend(loc='upper right')
    ax6.grid(True)

    plt.tight_layout()
    plt.savefig(__file__.replace('.py', '_AB42.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    # Plot 1: Oligomers
    ax1 = axes[0, 0]
    oligomer_sum1 = result1['[AB40_O2_ISF]']
    for i in range(3, 18):
        oligomer_sum1 += result1[f'[AB40_O{i}_ISF]']
    ax1.plot(time_years1, oligomer_sum1, label='Oligomers ApoE', linewidth=2,linestyle='--',color='red')

    oligomer_weighted_sum1 = result1['[AB40_O2_ISF]'] * 1
    for i in range(3, 18):
        oligomer_weighted_sum1 += result1[f'[AB40_O{i}_ISF]'] * (i-1)

    oligomer_sum2 = result2['[AB40_O2_ISF]']
    for i in range(3, 18):
        oligomer_sum2 += result2[f'[AB40_O{i}_ISF]']
    ax1.plot(time_years2, oligomer_sum2, label='Oligomers non-ApoE', linewidth=2,linestyle='--',color='blue')

    oligomer_weighted_sum2 = result2['[AB40_O2_ISF]'] * 1
    for i in range(3, 18):
        oligomer_weighted_sum2 += result2[f'[AB40_O{i}_ISF]'] * (i-1)
    ax1.plot(time_years1, oligomer_weighted_sum1, label='Oligomers weighted ApoE', linewidth=2,color='red')
    ax1.plot(time_years2, oligomer_weighted_sum2, label='Oligomers weighted non-ApoE', linewidth=2,color='blue')
    ax1.axvline(x=70, color='black', linestyle='--', linewidth=1.5)
    ax1.axvline(x=74, color='black', linestyle='--', linewidth=1.5)
    ...
    # Plot 2: Proto
    ax2 = axes[0, 1]
    proto_sum1 = result1['[AB40_O18_ISF]']
    for i in range(19, 25):
        proto_sum1 += result1[f'[AB40_O{i}_ISF]']
    ax2.plot(time_years1, proto_sum1, label='Proto ApoE', linewidth=2,linestyle='--',color='red')

    proto_weighted_sum1 = result1['[AB40_O18_ISF]'] * 17
    for i in range(19, 25):
        proto_weighted_sum1 += result1[f'[AB40_O{i}_ISF]'] * (i-1)
    ax2.plot(time_years1, proto_weighted_sum1, label='Proto weighted ApoE', linewidth=2,color='red')

    proto_sum2 = result2['[AB40_O18_ISF]']
    for i in range(19, 25):
        proto_sum2 += result2[f'[AB40_O{i}_ISF]']
    ax2.plot(time_years2, proto_sum2, label='Proto non-ApoE', linewidth=2,linestyle='--',color='blue')

    proto_weighted_sum2 = result2['[AB40_O18_ISF]'] * 17
    for i in range(19, 25):
        proto_weighted_sum2 += result2[f'[AB40_O{i}_ISF]'] * (i-1)
    ax2.plot(time_years2, proto_weighted_sum2, label='Proto weighted non-ApoE', linewidth=2,color='blue')
    ...
    # Plot 3: SUVR
    ax3 = axes[1, 0]
    plaque_sum1 = result1['[AB40_O25_ISF]']
    suvr_values1 = suvr(oligomer_weighted_sum1, proto_weighted_sum1, plaque_sum1)
    ax3.plot(time_years1, suvr_values1, label='SUVR ApoE', linewidth=2,color='red')

    plaque_sum2 = result2['[AB40_O25_ISF]']
    suvr_values2 = suvr(oligomer_weighted_sum2, proto_weighted_sum2, plaque_sum2)
    ax3.plot(time_years2, suvr_values2, label='SUVR non-ApoE', linewidth=2,color='blue')
    ...
    # Plot 4: AB42_O1_ISF and AB42_O25_ISF
    ax4 = axes[1, 1]
    ax4.plot(time_years1, result1['[AB40_O1_ISF]'], label='AB40_O1_ISF ApoE', linewidth=2,color='red')
    ax4.plot(time_years2, result2['[AB40_O1_ISF]'], label='AB40_O1_ISF non-ApoE', linewidth=2,color='blue')
    ax4.axvline(x=70, color='black', linestyle='--', linewidth=1.5)
    ax4.plot(csv_data_3C_ApoE['time']/24/365, csv_data_3C_ApoE['measurement'], 'r.', label='ApoE published', markersize=4)
    ax4.plot(csv_data_3C_nonApoE['time']/24/365, csv_data_3C_nonApoE['measurement'], 'b.', label='non-ApoE published', markersize=4)
    ax4.set_xlabel('Time (years)')
    ax4.set_ylabel('Concentration (nM)')
    ax4.set_title('AB40_O1_ISF and AB40_O1_CSF')
    ax4.legend(loc='upper right')
    ax4.grid(True)
...
# Plot 5: Plasma AB42_O1_central/AB40_O1_central
    ax5 = axes[2, 0]
    ax5.plot(time_years1, result1['[AB42_O1_SAS]']/result1['[AB40_O1_SAS]'], label='AB42_O1_SAS/AB40_O1_SAS ApoE', linewidth=2,color='red')
    ax5.plot(time_years2, result2['[AB42_O1_SAS]']/result2['[AB40_O1_SAS]'], label='AB42_O1_SAS/AB40_O1_SAS non-ApoE', linewidth=2,color='blue')
    ax5.plot(csv_data_3B_ApoE['time']/24/365, csv_data_3B_ApoE['measurement'], 'r.', label='ApoE published', markersize=4)
    ax5.plot(csv_data_3B_nonApoE['time']/24/365, csv_data_3B_nonApoE['measurement'], 'b.', label='non-ApoE published', markersize=4)
    ax5.axvline(x=70, color='black', linestyle='--', linewidth=1.5)
    ax5.set_xlabel('Time (years)')
    ax5.legend(loc='upper right', fontsize=7)
    ax5.set_ylabel('Ratio CSF AB42/AB40')

    # Plot 6: AB40_plaque
    ax6 = axes[2, 1]
    ax6.plot(time_years1, result1['[AB40_O25_ISF]'], label='AB40_O25_ISF ApoE', linewidth=2,color='red')
    ax6.plot(time_years2, result2['[AB40_O25_ISF]'], label='AB40_O25_ISF non-ApoE', linewidth=2,color='blue')
    ax6.axvline(x=70, color='black', linestyle='--', linewidth=1.5)
    ax6.axvline(x=74, color='black', linestyle='--', linewidth=1.5)
    ax6.plot([70], [5102], 'o', color='green', markersize=6)
    ax6.plot([74], [6028], 'o', color='green', markersize=6)
    ...
    plt.tight_layout()
    plt.savefig(__file__.replace('.py', '_AB40.png'), dpi=300, bbox_inches='tight')
    plt.show()

# Main execution
if __name__ == "__main__":
    # Run optimization and simulation
    r, result1,result2, csv_data_3C_ApoE, csv_data_3C_nonApoE, csv_data_3A_ApoE, csv_data_3A_nonApoE, suvr, csv_data_3B_ApoE, csv_data_3B_nonApoE = run_optimization_and_simulation()
