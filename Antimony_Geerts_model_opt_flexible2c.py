...
def run_optimization_and_simulation():
    ...
    def create_objective(csv_data_3C_ApoE, csv_data_3C_nonApoE, csv_data_3A_ApoE, csv_data_3A_nonApoE, r, param_names,csv_data_7_proto_placebo,csv_data_7_plaque_placebo,csv_data_7_suvr_placebo,csv_data_7_oligomer_placebo):
        def objective(params):
            ...
            if result1 is None:
                return 0
            for i, time in enumerate(csv_data_7_plaque_placebo['time'].values):
                plaque_results = calculate_suvr_at_year(result1, suvr, time)
                mse1 = mse1 + (plaque_results['plaque_sum']-csv_data_7_plaque_placebo['measurement'].values[i])**2 / csv_data_7_plaque_placebo['measurement'].values[i] /10
                # print(f"time: {time}, model plaque: {plaque_results['plaque_sum']}, data plaque: {csv_data_7_plaque_placebo['measurement'].values[i]}")
            for i, time in enumerate(csv_data_7_suvr_placebo['time'].values):
                suvr_results = calculate_suvr_at_year(result1, suvr, time)
                mse1 = mse1 + (suvr_results['suvr_at_year']-csv_data_7_suvr_placebo['measurement'].values[i])**2 / csv_data_7_suvr_placebo['measurement'].values[i]
            for i, time in enumerate(csv_data_7_oligomer_placebo['time'].values):
                oligomer_results = calculate_suvr_at_year(result1, suvr, time)
                mse1 = mse1 + (oligomer_results['oligomer_weighted_sum']-csv_data_7_oligomer_placebo['measurement'].values[i])**2 / csv_data_7_oligomer_placebo['measurement'].values[i] / 10
            for i, time in enumerate(csv_data_7_proto_placebo['time'].values):
                proto_results = calculate_suvr_at_year(result1, suvr, time)
                mse1 = mse1 + (proto_results['proto_weighted_sum']-csv_data_7_proto_placebo['measurement'].values[i])**2 / csv_data_7_proto_placebo['measurement'].values[i] / 10
            ...
            if result2 is None:
                return 0
            for i, time in enumerate(csv_data_7_plaque_placebo['time'].values):
                plaque_results = calculate_suvr_at_year(result2, suvr, time)
                mse2 = mse2 + (plaque_results['plaque_sum']-csv_data_7_plaque_placebo['measurement'].values[i])**2 / csv_data_7_plaque_placebo['measurement'].values[i] /10
            # print(f"model plaque: {plaque_results['plaque_sum']}, data plaque: {csv_data_7_plaque_placebo['measurement'].values[i]}")
           
            for i, time in enumerate(csv_data_7_suvr_placebo['time'].values):
                suvr_results = calculate_suvr_at_year(result2, suvr, time)
                mse2 = mse2 + (suvr_results['suvr_at_year']-csv_data_7_suvr_placebo['measurement'].values[i])**2 / csv_data_7_suvr_placebo['measurement'].values[i]
            for i, time in enumerate(csv_data_7_oligomer_placebo['time'].values):
                oligomer_results = calculate_suvr_at_year(result2, suvr, time)
                mse2 = mse2 + (oligomer_results['oligomer_weighted_sum']-csv_data_7_oligomer_placebo['measurement'].values[i])**2 / csv_data_7_oligomer_placebo['measurement'].values[i]
            for i, time in enumerate(csv_data_7_proto_placebo['time'].values):
                proto_results = calculate_suvr_at_year(result2, suvr, time)
                mse2 = mse2 + (proto_results['proto_weighted_sum']-csv_data_7_proto_placebo['measurement'].values[i])**2 / csv_data_7_proto_placebo['measurement'].values[i] / 10
    ...
    objective = create_objective(csv_data_3C_ApoE, csv_data_3C_nonApoE, csv_data_3A_ApoE, csv_data_3A_nonApoE, r, param_names,csv_data_7_proto_placebo,csv_data_7_plaque_placebo,csv_data_7_suvr_placebo,csv_data_7_oligomer_placebo)
...
def create_plots(r, result1, result2, csv_data_3C_ApoE, csv_data_3C_nonApoE, csv_data_3A_ApoE, csv_data_3A_nonApoE, suvr):
    ...
    ax3.plot(csv_data_3A_ApoE['time']/24/365, csv_data_3A_ApoE['measurement'], 'r.', label='ApoE', markersize=4)
    ax3.plot(csv_data_3A_nonApoE['time']/24/365, csv_data_3A_nonApoE['measurement'], 'b.', label='non-ApoE', markersize=4)
    ...
    ax4.plot(csv_data_3C_ApoE['time']/24/365, csv_data_3C_ApoE['measurement'], 'r.', label='ApoE', markersize=4)
    ax4.plot(csv_data_3C_nonApoE['time']/24/365, csv_data_3C_nonApoE['measurement'], 'b.', label='non-ApoE', markersize=4)
