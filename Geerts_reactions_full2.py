...
def build_reactions():
    ...
    for Species in ['AB40', 'AB42']:
        for Comp in ['ISF','PVS','BBB','BCSFB','BrainPlasma','central','CM','LV','CSFSAS','TFV','SAS' ]:
            counter += 1
    ...
    for Species in ['AB40', 'AB42']:
        for n in range(2, 25):
            for Comp in ['ISF']:    
                ...
                Products = f"[0]"
                Rate_type = "MA"
                Rate_eqtn_prototype = f"Microglia*(10.0*Microglia_high_frac*Microglia_Vmax_{Species}/(Microglia_EC50_{Species} + {Species}_O{n}_{Comp}) + (1.0 - Microglia_high_frac)*Microglia_Vmax_{Species}/(Microglia_EC50_{Species} + {Species}_O{n}_{Comp}))"            
                Reaction_dict = {"Reaction_name": Reaction_name,"Reactants": Reactants,"Products": Products,"Rate_type": Rate_type,"Rate_eqtn_prototype": Rate_eqtn_prototype,}
                all_reactions.append(Reaction_dict)
    ...
    for Species in ['AB40', 'AB42']:
        for Comp in ['ISF']:
            ...
            Products = f"[{Species}_O1_{Comp}]"
            Rate_type = "MA"
            Rate_eqtn_prototype = f"k_in_{Species}"            
            Reaction_dict = {"Reaction_name": Reaction_name,"Reactants": Reactants,"Products": Products,"Rate_type": Rate_type,"Rate_eqtn_prototype": Rate_eqtn_prototype,}
            all_reactions.append(Reaction_dict)
    
    for Species in ['AB40', 'AB42']:
        for Comp in [['ISF','PVS'],['PVS','central']]:
            for n in range(2, 25):
                counter += 1
                Comp1 = Comp[0]
                Comp2 = Comp[1]
                Reaction_name = f"Flow ISF to PVS Abeta"
                Reactants = f"[{Species}_O{n}_{Comp1}]"
                Products = f"[{Species}_O{n}_{Comp2}]"
                Rate_type = "UDF"
                match Comp:
                    case ['ISF','PVS']:
                        Rate_eqtn_prototype = f"(1.0 - sigma_{Comp1}_{Comp2}_O{n}) * Q_PVS"
                    case ['PVS','central']:
                        Rate_eqtn_prototype = f"(1.0 - sigma_PVS_Abeta) * Q_PVS"
                Reaction_dict = {"Reaction_name": Reaction_name,"Reactants": Reactants,"Products": Products,"Rate_type": Rate_type,"Rate_eqtn_prototype": Rate_eqtn_prototype,}
                all_reactions.append(Reaction_dict)
            ...
            match Comp:
                case ['ISF','PVS']:
                    ...
                    if Species == 'AB40_O1' or Species == 'AB42_O1' :
                        Rate_eqtn_prototype = f"(1.0 - sigma_{Comp1}_{Comp2}_O1) * Q_{Comp1}"
                    elif Species == 'Antibody':
                        Rate_eqtn_prototype = f"(1.0 - sigma_{Comp1}_{Comp2}_Antibody) * Q_{Comp1}"
                    else:
                        Rate_eqtn_prototype = f"0"
                case ['PVS','central']:
                    Reaction_name = f"Flow PVS to central Abeta"
                    if Species == 'Antibody':
                        Rate_eqtn_prototype = f"(1.0 - sigma_PVS_Antibody) * Q_PVS"
                    else:
                        Rate_eqtn_prototype = f"(1.0 - sigma_PVS_Abeta) * Q_PVS"
                case ['ISF','central']: 
                    Reaction_name = f"Flow ISF to central Abeta"
                    if Species == 'AB40_O1' or Species == 'AB42_O1':
                        Rate_eqtn_prototype = f"(1.0 - sigma_{Comp1}_lymph_Abeta) * Qbrain_ISF"
                    else:
                        Rate_eqtn_prototype = f"(1.0 - sigma_{Comp1}_lymph_Antibody) * Qbrain_ISF"
                case ['ISF','LV']:
                    Reaction_name = f"Flow ISF to LV Abeta"
                    Rate_eqtn_prototype = f"f_LV*Qbrain_ISF"                  
                case ['ISF','TFV']:
                    Reaction_name = f"Flow ISF to TFV Abeta"
                    Rate_eqtn_prototype = f"(1.0 - f_LV) * Qbrain_ISF"
                case ['BrainPlasma','ISF']:
                    Reaction_name = f"Flow BrainPlasma to ISF Abeta"
                    Rate_eqtn_prototype = f"(1.0 - sigma_vascular_ISF)*Qbrain_ISF"
                case ['BrainPlasma','LV']:
                    Reaction_name = f"Flow BrainPlasma to LV Abeta"
                    Rate_eqtn_prototype = f"f_LV*(1.0 - sigma_vascular_BCSFB)*Qbrain_CSF"
                case ['BrainPlasma','TFV']: 
                    Reaction_name = f"Flow BrainPlasma to TFV Abeta"
                    Rate_eqtn_prototype = f"(1.0 - f_LV)*(1.0 - sigma_vascular_BCSFB)*Qbrain_CSF"
                case ['LV','TFV']: 
                    Reaction_name = f"Flow LV to TFV Abeta"
                    Rate_eqtn_prototype = f"f_LV*(Qbrain_CSF + Qbrain_ISF)"
                case ['TFV','CM']:
                    Reaction_name = f"Flow TFV to CM Abeta"
                    Rate_eqtn_prototype = f"(Qbrain_CSF + Qbrain_ISF)"
                case ['CM','SAS']:
                    Reaction_name = f"Flow CM to SAS Abeta"
                    Rate_eqtn_prototype = f"(Qbrain_CSF + Qbrain_ISF)"
                case ['SAS','ISF']:
                    Reaction_name = f"Flow SAS to ISF Abeta"
                    Rate_eqtn_prototype = f"Qbrain_ISF"
                case ['SAS','central']:
                    Reaction_name = f"Flow SAS to central Abeta"
                    if Species == 'AB40_O1' or Species == 'AB42_O1':
                        Rate_eqtn_prototype = f"(1 - sigma_SAS_lymph_Abeta)*Qbrain_CSF"
                    else:
                        Rate_eqtn_prototype = f"(1 - sigma_SAS_lymph_Antibody)*Qbrain_CSF"
                case ['BrainPlasma','central']:
                    Reaction_name = f"Flow BrainPlasma to central Abeta"
                    Rate_eqtn_prototype = f"(Qbrain_plasma - Qlymph_Brain)"
